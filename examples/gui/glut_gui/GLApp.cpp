...
GLApp::GLApp()
    : m_winName(std::string("PhysIKA 3.0")), m_winID(-1), m_width(640), m_height(480), display_fps_(true), screen_capture_file_index_(0), event_mode_(false), m_bAnimate(false), m_secLineNum(10), m_bShowBackground(true)
{
...
void GLApp::createWindow(int width, int height)
{
    ...
    std::string win_title = std::string("Physika");
...
void GLApp::motionFunction(int x, int y)
{
    ...
    else if (window->getButtonType() == GLUT_RIGHT_BUTTON)
    {
        ...
        {
            // Move camera.
            activeCamera.localTranslate(Vec3f(movex, movey, 0));
        }
        else
        {
            // Fix camera focus .
            // And move camera forward or backward.
            activeCamera.localTranslate(Vec3f(0, 0, movey));
...
void GLApp::drawBoundingBox(Vec3f lo, Vec3f hi)
{
    glPushMatrix();
