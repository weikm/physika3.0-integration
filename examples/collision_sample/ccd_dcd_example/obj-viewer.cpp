...
void CalculateFrameRate()
{
	...
	if (delta > 1.0f) {
		...
		sprintf(fpsBuffer, "ccd: %s, speed: %s, FPS: %d", gCCD ? "true" : "false", speed == 0 ? "slow" : ((speed == 1) ? "normal" : "fast"), int(ceil(framesPerSecond)));
		framesPerSecond = 0;
	}
...
void draw()
{
	...
	drawModel(!b['t'], !b['p'], false, b['e'], level);
...
#if 1
	if (idx <= 500) {
		sprintf(buffer, "h:\\temp\\objout\\%05d.obj", idx++);
		exportObj(buffer);
	}
	return;
#endif

	sprintf(buffer, "c:\\temp\\frame-%05d.scn", idx++);
	exportModel(buffer);
}
...
{    
	...
	glutAddMenuEntry("Toggle ccd [c]", 'c');
	glutAddMenuEntry("Reset scene [r]", 'r');
	glutAddMenuEntry("Adjust speed [s]", 's');
...
int main(int argc, char **argv)
{
	...
	initOpengl();
	//initModel(argv[1]);
	//initModel("my-bunny.obj", "my-stairs1.obj");
	//initModel("my-bunny.obj", "terrain2.obj");
	//initModel("rocks/05/model.obj", "terrain2.obj");
	initModel("rocks/05/model.obj", "terrain-flat.obj");
	key3();
