...
vec3f areaColor(vec3f& x0, vec3f& x1, vec3f& x2, REAL areaIn)
{
    if (areaIn < 0)
        return vec3f(0.5, 0.5, 0.4);

    vec3f tmp  = (x1 - x0).cross(x2 - x0);
    REAL  area = sqrt(tmp.dot(tmp)) * 0.5;
...
void mesh::display(bool t, bool p, bool e, int level, bool rigid, set<int>& ids, vector<int>& vids, int id)
{
    glPointSize(10.f);
...
#ifdef USE_DOUBLE
    if (dl)
        glVertexPointer(3, GL_DOUBLE, sizeof(REAL) * 3, _ivtxs);
    else
        glVertexPointer(3, GL_DOUBLE, sizeof(REAL) * 3, _vtxs);
...
#else
    if (dl)
        glVertexPointer(3, GL_FLOAT, sizeof(REAL) * 3, _ivtxs);
    else
        glVertexPointer(3, GL_FLOAT, sizeof(REAL) * 3, _vtxs);
...
void mesh::povray(char* fname, bool first)  // output to povray
{
    FILE* fp = fopen(fname, "wt");
    if (fp == NULL)
    {
...
static const REAL scale  = 1.f;
static const REAL kScale = 1.f;
...
inline REAL projectPointOntoLine(const vec3f& p, const vec3f& a, const vec3f& b)
{
    vec3f ba = b - a;
    vec3f pa = p - a;
...
inline void findClosestPointsEE(const vec3f& x1, const vec3f& x2, const vec3f& x3, const vec3f& x4, REAL& w1, REAL& w2)
{
    vec3f x21 = x2 - x1;
    REAL  a   = x21.dot(x21);
    ...
    w1 = REAL((e * c - b * f) / (a * c - b * b));
    w2 = REAL((f - b * w1) / c);
}

// calculates the distance of 2 edges
inline REAL edgeEdgeDistance(const vec3f& np11, const vec3f& np12, const vec3f& np21, const vec3f& np22, REAL& out_a1, REAL& out_a2, vec3f& out_normal)
{
    vec3f temp, temp2;
    ...
    out_normal  = line1.cross(line2);
    REAL length = out_normal.dot(out_normal);
...
void drawEdges(bool a, bool b)
{
    ...
    if (first)
    {
        ...
        REAL dist = edgeEdgeDistance(x1, x2, x3, x4, a, b, n);
