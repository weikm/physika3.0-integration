...
extern void getManifoldGPU(void* data, int num);
extern void clearManifoldsGPU();

extern int checkTriTriCDGPU(const void* offIn, const void* rotIn, REAL margin, REAL x0, REAL y0, REAL z0, REAL x1, REAL y1, REAL z1, REAL x2, REAL y2, REAL z2);
...
extern int checkRigidRigidManifoldGPU(REAL margin);
    ...
    vec3f*       clipped)
{
    ...
    for (int i = 1; i < polygon_point_count; i++)
    {
        ...
        get_plane_clip_polygon_collect(
            polygon_points[i - 1], polygon_points[i], olddist, dist, clipped, clipped_count);
    ...
    get_plane_clip_polygon_collect(
        polygon_points[polygon_point_count - 1], polygon_points[0], olddist, firstdist, clipped, clipped_count);
...
)
{
    ...
    get_plane_clip_polygon_collect(
        point0, point1, olddist, dist, clipped, clipped_count);
    ...
    get_plane_clip_polygon_collect(
        point1, point2, olddist, dist, clipped, clipped_count);
    olddist = dist;
...
class cscene
{
    ...
    std::vector<crigid*>   _rigids, _init_rigids, _prev_rigids;
...
public:
    ...
    void backupState(std::vector<crigid*>& backup)
    {
        backup.clear();
        for (auto r : _rigids)
            backup.push_back(new crigid(*r));
    }
    void restoreState(std::vector<crigid*>& backup)
    {
        for (auto r : _rigids)
            delete r;
        ...
        _rigids.clear();
        for (auto r : backup)
            _rigids.push_back(new crigid(*r));
    }

    void backupPrevStep()
    {
        backupState(_prev_rigids);
    }

    void restorePrevStep()
    {
        restoreState(_prev_rigids);
    }

    void backupInit()
    {
        backupState(_init_rigids);
    }

    void restoreInit()
    {
        restoreState(_init_rigids);
    }

    FORCEINLINE const crigid* getRigid(int rid)
    {
    ...
    void draw(int level)
    {
        ...
        {
            auto r = _rigids[i];
            drawRigid(r, i == _rigids.size() - 1, level, vec3f());
        }
    ...
    bool exportObj(const char* fname)
    {
        ...
        for (int i = 0; i < _rigids.size(); i++)
        {
            // if (_rigids[i]->isActive() == true)
            vtxOffset += _rigids[i]->exportObj(fp, vtxOffset, i);
        }
    ...
    //////////////////////////////////////////////////////////////////////////////
    int stepSimulation(float timeStep, bool ccd, int numSimulationSubSteps = 1)
    {
#if 0
        ...
        while (!done)
        {
            double dt = timeStep / numSimulationSubSteps;
            done      = false;

            saveKinematicState(timeStep);
            applyGravity();

            bool findPenetration = false;
            for (int i = 0; i < numSimulationSubSteps; i++)
            {
                bool ret = internalSingleStepSimulation(dt, ccd);
                if (ret == false)
                {
                    std::cout << "false" << std::endl;
                    restorePrevStep();
                    numSimulationSubSteps++;
                    findPenetration = true;
                    break;
                }
                synchronizeMotionStates();
            }
        ...
        if (numSimulationSubSteps > 1)
            printf("###### numSimulationSubSteps = %d\n", numSimulationSubSteps);
    ...
    bool internalSingleStepSimulation(float timeStep, bool ccd)
    {
        {
        ...
        // all contact related...
        {
            ...
            /// perform collision detection
            {
                ...
                if (false == performDiscreteCollisionDetection())
                    return false;
            }
            {
        ...
        if (ccd)
            return performContinuousCollisionDetection();
        else
            return true;
    }
                         ...
                         float                           timeStep)
    {
        ...
        for (int i = 0; i < numManifolds; i++)
        {
            ...
            const crigid* rig  = getRigid(mf->getBody1());
            const crigid* rig0 = getRigid(mf->getBody0());
            ...
            for (int j = 0; j < mf->getNumContacts(); j++)
            {
                ...
                {
                    addContactConstraints(mf, cp, contactConstraintPool, frictionConstraintPool, timeStep, solverBodyIdA, solverBodyIdB, solverBodyA, solverBodyB, bodyPool, rig0, rig, rollingFriction);
                }
            }
        }
    }

    void addContactConstraints(manifold* mf, manifoldPoint& cp, std::vector<csolverConstraint>& contactConstraintPool, std::vector<csolverConstraint>& frictionConstraintPool, float timeStep, int solverBodyIdA, int solverBodyIdB, csolverBody* solverBodyA, csolverBody* solverBodyB, std::vector<csolverBody>& bodyPool, const crigid* bodyA, const crigid* bodyB, int& rollingFriction)
    {
        vec3f rel_pos1;
        vec3f rel_pos2;
        float relaxation;
        ...
        // if (!cp.m_lateralFrictionInitialized)
        {
            ...
            if (!(solverMode & SOLVER_DISABLE_VELOCITY_DEPENDENT_FRICTION_DIRECTION) && lat_rel_vel > FLT_EPSILON)
            {
                ...
                if ((solverMode & SOLVER_USE_2_FRICTION_DIRECTIONS))
                {
                    ...
                    addFrictionConstraint(cp.m_lateralFrictionDir2, solverBodyIdA, solverBodyIdB, frictionIndex, cp, rel_pos1, rel_pos2, bodyA, bodyB, relaxation, 0, 0, frictionConstraintPool, bodyPool);
                }
                ...
                addFrictionConstraint(cp.m_lateralFrictionDir1, solverBodyIdA, solverBodyIdB, frictionIndex, cp, rel_pos1, rel_pos2, bodyA, bodyB, relaxation, 0, 0, frictionConstraintPool, bodyPool);
            }
            else
            {
    ...
    void addFrictionConstraint(const vec3f& normalAxis, int solverBodyIdA, int solverBodyIdB, int frictionIndex, manifoldPoint& cp, const vec3f& rel_pos1, const vec3f& rel_pos2, const crigid* colObj0, const crigid* colObj1, float relaxation, float desiredVelocity, float cfmSlip, std::vector<csolverConstraint>& frictionConstraintPool, std::vector<csolverBody>& bodyPool)
    {
        csolverConstraint solverConstraint;
        solverConstraint.m_frictionIndex = frictionIndex;
        setupFrictionConstraint(solverConstraint, normalAxis, solverBodyIdA, solverBodyIdB, cp, rel_pos1, rel_pos2, colObj0, colObj1, relaxation, desiredVelocity, cfmSlip, bodyPool);
        frictionConstraintPool.push_back(solverConstraint);
    }

    void setupFrictionConstraint(csolverConstraint& solverConstraint, const vec3f& normalAxis, int solverBodyIdA, int solverBodyIdB, manifoldPoint& cp, const vec3f& rel_pos1, const vec3f& rel_pos2, const crigid* colObj0, const crigid* colObj1, float relaxation, float desiredVelocity, float cfmSlip, std::vector<csolverBody>& bodyPool)
    {
    ...
    bool performContinuousCollisionDetection()
    {
        {
        ...
        }
        {
            ...
            computeOverlappingPairs(true);
        }
        {
            myTimer tr("\t\t#processAllOverlappingPairs");
            return processAllOverlappingPairs(true);
        }
    }

    // DCD with current rigid body, not the predicted ...
    bool performDiscreteCollisionDetection()
    {
        {
            myTimer tr("\t\t#updateAabbs");
            updateAabbs(false);
        }
        {
            ...
            // Á½Á½rigid×öÅö×²
            myTimer tr("\t\t#computeOverlappingPairs");
            computeOverlappingPairs(false);
        }
        {
            // narrow phase
            myTimer tr("\t\t#processAllOverlappingPairs");
            return processAllOverlappingPairs(false);
        }
    }

    void updateAabbs(bool ccd)
    {
        for (auto body : _rigids)
        {
            body->updateContactBx(gContactBreakingThreshold, ccd);
        }
    }

    // a naive broadphase culling, should be imporved later
    void computeOverlappingPairs(bool ccd)
    {
    ...
    bool processAllOverlappingPairs(bool ccd)
    {
#ifdef GPU
// #define OLD_GPU
#ifdef OLD_GPU
        ...
        RigidRigidManifoldGPU(maxTriPairs);
#ifdef PROF
#else
        printf("\t\t\t$$$rigid-pair-size = %zd\n", _rigid_pairs.size());
        printf("\t\t\t$$$maxTriPairs = %d\n", maxTriPairs);
...
#pragma omp parallel for
            for (int i = 0; i < pairNum; i++)
            {
                ...
                unsigned int id0, id1;
                pair.get(id0, id1);
                if (ccd == false && false == RigidRigidCollision(_rigids[id0], _rigids[id1], tr1, tr2, maxL, maxTriPairs))
                    return false;

                if (ccd && false == RigidRigidContinuousCollision(_rigids[id0], _rigids[id1]))
                    return false;
            }
...
#ifdef GPU
    void RigidRigidManifoldGPU(int& maxP)
    {
        if (sManifoldSet == nullptr)
        {
            sManifoldSet = new manifold[1000];
        }
        ...
        int pairNum;
        {
            ...
            pairNum = checkRigidRigidManifoldGPU(margin);
            if (pairNum)
            {
                getManifoldGPU(sManifoldSet, pairNum);
            }
            gpuTimerEnd("checkRigidRigidManifoldGPU", true);
        }
...

        {
            ...
            for (int i = 0; i < pairNum; i++)
            {
                _predictiveManifolds.push_back(new manifold(sManifoldSet[i]));
            }
        }
    }
...
#pragma omp parallel for
    ...
    void RigidRigidCollision(crigid* ra, crigid* rb, myTimer2& tr1, myTimer2& tr2, int& maxL, int& maxP)
    {
        std::vector<id_pair> pairset;
        {
            ...
            int pairNum = checkTriTriCDGPU(&trfA2B.getOrigin(), &trfA2B.getBasis(), margin, bMin.x, bMin.y, bMin.z, bMax.x, bMax.y, bMax.z, quan.x, quan.y, quan.z);
...
#else
    void RigidRigidCollisionSlower(crigid* ra, crigid* rb, myTimer2& tr1, myTimer2& tr2, int& maxL, int& maxP)
    {
#if 0
		btPairSet pairset;
    ...
    bool RigidRigidCCD(crigid* body, crigid* base)
    {
        // check vertex/faces collisions between the body and the base
        // if found, return true
        // else, return false
        const transf& trfA    = body->getTrf();
        const transf& trfA0   = body->getTrf0();
        const transf& trfB    = base->getTrf();
        const transf  trfA2B  = trfB.inverse() * trfA;
        const transf  trfA02B = trfB.inverse() * trfA0;

        vec3f* vtxs   = body->getMesh()->getVtxs();
        int    vtxNum = body->getMesh()->getNbVertices();

        tri3f* tris   = base->getMesh()->getTris();
        int    triNum = base->getMesh()->getNbFaces();
        vec3f* vtxsB  = base->getMesh()->getVtxs();
        ...
        {
            vec3f& v  = vtxs[i];
            vec3f  v0 = trfA02B.getVertex(v);
            vec3f  v1 = trfA2B.getVertex(v);
            BOX    ba(v0);
            ba += v1;
            ...
            {
                tri3f& t  = tris[j];
                vec3f  va = vtxsB[t.id0()];
                vec3f  vb = vtxsB[t.id1()];
                vec3f  vc = vtxsB[t.id2()];
                BOX    bb(va);
                bb += vb;
                bb += vc;

                if (false == ba.overlaps(bb))
                    continue;

                REAL rt;
                if (ccdVFtest(v0, v1, va, vb, vc, rt))
                    find++;

                if (find > vtxNum * 0.5)
                    // if (find	> vtxNum*0.1) //slower
                    return true;
            }
        }
        return false;
    ...
    bool RigidRigidContinuousCollision(crigid* ra, crigid* rb)
    {
        if (ra == _rigids.back())
            if (RigidRigidCCD(rb, ra) == true)
                return false;

        if (rb == _rigids.back())
            if (RigidRigidCCD(ra, rb) == true)
                return false;
        return true;
    }

    bool RigidRigidCollision(crigid* ra, crigid* rb, myTimer2& tr1, myTimer2& tr2, int& maxL, int& maxP)
    {
        std::vector<id_pair> pairset;
        {
            double           t0   = omp_get_wtime();
        ...
        {
#if 0
			//DCD returns null, we still need check CCD here to avoid tunneling
			//will do these checks in performContinuousCollisionDetection
			if (ra == _rigids.back())
				if (RigidRigidCCD(rb, ra) == true)
					return false;
    ...
    void collide_sat_triangles(crigid* ra, crigid* rb, unsigned int ta, unsigned int tb)
    {
        ...
        if (ptri0.overlap_test_conservative(ptri1))
        {
            if (ptri0.find_triangle_collision_clip_method(ptri1, contact_data))
            {
                ...
                while (j--)
                {
                    addContactPoint(ra, nullptr, rb, contact_data.m_points[j], contact_data.m_separating_normal.xyz(), -contact_data.m_penetration_depth);
                }
            }
        }
    }
...
#else
            const crigid* mfRig0 = getRigid(mf->getBody0());
            const cplane* mfPln  = getPlane(mf->getPlane());
            const crigid* mfRig  = getRigid(mf->getBody1());
#endif
...
#else
        manifold* mf = new manifold(getRigidID(r0), getPlaneID(pln), getRigidID(r), 0, contactBreakingThreshold, contactProcessingThreshold);
#endif
...
kmesh* initBunny(const char* ofile, REAL scale = 1.0)
{
...
crigid* putMesh(kmesh* km, const vec3f& startPos)
{
    float mass = 4.f;
...
void initStairs(kmesh* ks)
{
    crigid* rig = putMesh(ks, vec3f(0, -50, 0));
    g_box += rig->bound();
    g_scene.addRigid(rig);
}

void initRocks(std::vector<kmesh*> kmRs, BOX& bx)
{
    // BOX bx = km->bound();
    float dx  = bx.width() * 1.2 * 0.6;
    float dy  = bx.height() * 1.2 * 0.6;
    float dz  = bx.depth() * 1.2 * 0.6;
    vec3f off = vec3f(-dx * 4 * 0.5, dy * 4, -dz * 4 * 0.5);

    // k=12 -> 300 bunnys, 2M triangles
    // k=6 -> 150 bunnys, 1M triangles
    int c = 0;
#if 0
	int i = 0;
	int j = 0;
    ...
    for (int i = 0; i < 5; i++)
        for (int j = 0; j < 5; j++)
...
#endif
            {
                ...
                crigid* rig = shootMesh(kmRs[c % (kmRs.size())], pos, dst);
                g_box += rig->bound();
                g_scene.addRigid(rig);
                c++;
            }
...
{
    if (gFlat)
        g_scene.addPlane(new cplane(vec3f(0, 1, 0), gFlatY));
    else
    {
...
void initModel(const char* cfile, const char* sfile)
{
    kmesh* kmB = initBunny(cfile, BUNNY_SCALE);
    kmesh* kmS = initBunny(sfile);

    std::vector<kmesh*> kmRocks;
    for (int i = 1; i <= 8; i++)
    {
        char buffer[512];
        strcpy(buffer, cfile);
        char* idx = strstr(buffer, "05");
        char  pat[512];
        sprintf(pat, "%02d", i);
        strncpy(idx, pat, 2);

        kmRocks.push_back(initBunny(buffer, BUNNY_SCALE));
    }

    // initRigids(kmB);
    // initPlanes();
    initRocks(kmRocks, kmB->bound());
    initStairs(kmS);
...
bool exportObj(const char* cfile)
{
    return g_scene.exportObj(cfile);
}
...
inline float getDt(int speed)
{
    switch (speed)
    {
        case 1:
            return float(5.0) / float(60.);
        case 2:
            return float(10.0) / float(60.0);
        default:
            return float(1.0) / float(60.0);
    }
}

bool dynamicModel(int speed)
{
    static int st = 0;
#if 0
	if (st == 39) {
		printf("here!\n");
		exportObj("c:\\temp\\out39.obj");
	}

	if (st == 40) {
		printf("here!\n");
		exportObj("c:\\temp\\out40.obj");
	}
#endif

    double tstart = omp_get_wtime();
    float  dt     = getDt(speed);  // float(10.) / float(60.);
    bool   ccd    = gCCD;

    g_scene.stepSimulation(dt, ccd, 1);
	...
	if (st == 40) {
		printf("here!\n");
		exportObj("c:\\temp\\out39after.obj");
	}
#endif
