...
#define MAX_MANIFOLD_NUM 2000
static g_manifold* sManifoldPtr = NULL;
static int* sManifoldIdx;

#define MAX_MINPT_NUM 2000
static REAL3* sMinPtPtr = NULL;
static int* sMinPtIdx = NULL;
...
void clearManifoldsGPU()
{
	if (sManifoldIdx != nullptr) {
		int dummyNum = 0;
		cudaMemcpy(sManifoldIdx, &dummyNum, sizeof(int), cudaMemcpyHostToDevice);
	}
}
...
void clearGPU()
{
	...
	if (sManifoldPtr) {
		cudaFree(sManifoldPtr);
		cudaFree(sManifoldIdx);
	}
...
void getManifoldGPU(void* data, int num)
{
	int msz = num * sizeof(g_manifold);
	cutilSafeCall(cudaMemcpy(data, sManifoldPtr, msz, cudaMemcpyDeviceToHost));
	printf("%d manifolds (%d bytes) feteched...\n ", num, msz);
}
...
CU_FORCEINLINE __device__ void get_primitive_triangle(g_tri3f *tris, REAL3 *vtxs, int prim_index, gprimitiveTriangle& triangle)
{
	g_tri3f& t = tris[prim_index];
...
CU_FORCEINLINE __device__ void PlaneSpace1(const REAL3& n, REAL3& p, REAL3& q)
{
...
CU_FORCEINLINE __device__ g_manifold* getManifold(uint2 rids, g_manifold* manifoldPtr, int* manifoldIdx, g_manifold*& ptr)
{
	if (ptr != nullptr)
		return ptr;

	uint offset = atomicAdd(manifoldIdx, 1);
	ptr = manifoldPtr + offset;

	ptr->set(rids);
	return ptr;
}

CU_FORCEINLINE __device__ void addContactPoint(
	uint2 rids, const g_transf& trfA, const g_transf& trfB,
	g_manifold* manifoldPtr, int* manifoldIdx, g_manifold*& ptr,
	REAL3 & ptW, REAL3 & nrmW, float depth)
{
	g_manifold* mf = getManifold(rids, manifoldPtr, manifoldIdx, ptr);
	if (depth > mf->getContactBreakingThreshold())
		return;

	REAL3 pointA = ptW + nrmW * depth;
	REAL3 localB =trfB.getVertexInv(ptW);
	REAL3 localA = pointA;
	localA = trfA.getVertexInv(localA);

	g_manifoldPoint newPt(localA, localB, nrmW, depth);
...
CU_FORCEINLINE __device__ void
updateManifold(int ta, int tb, uint2 rids, g_manifold* manifoldPtr, int* manifoldIdx, g_manifold *&ptr, g_tri3f* tris, REAL3* vtxs, const g_transf& trfA, const g_transf& trfB)
{
#if 0
	if (ta == 864 && tb == 19941 && rids.x == 0 && rids.y == 1)
		rids.x = 0;
#endif

	gprimitiveTriangle ptri0;
	gprimitiveTriangle ptri1;
	gtriangleContact contact_data;

	get_primitive_triangle(tris, vtxs, ta, ptri0);
	get_primitive_triangle(tris, vtxs, tb, ptri1);
	ptri0.applyTransform(trfA);
	ptri1.applyTransform(trfB);
	//build planes
	ptri0.buildTriPlane();
	ptri1.buildTriPlane();
	// test conservative
	if (ptri0.overlap_test_conservative(ptri1))
	{
		if (ptri0.find_triangle_collision_clip_method(ptri1, contact_data))
		{
			...
			while (j--)
			{
				REAL4& n = contact_data.m_separating_normal;
				REAL3 nrm = make_REAL3(n.x, n.y, n.z);

				addContactPoint(rids, trfA, trfB,
					manifoldPtr, manifoldIdx, ptr,
					contact_data.m_points[j], nrm, -contact_data.m_penetration_depth);
			}
		}
	}
}
...
CU_FORCEINLINE __device__ void
bvhQuery2(int idx, const g_box2boxTrf& trf,
	g_tri3f* tris, REAL3* vtxs, g_bvh_node* root,
	uint2 rids, g_manifold* manifoldPtr, int* manifoldIdx, const g_transf &trfA, const g_transf &trfB)
{
	const g_bvh_node* stack[STACK_SIZE * 2];
	const g_bvh_node* nodeA = root;
	const g_bvh_node* nodeB = root;
	unsigned int stackPtr = 0;
	g_manifold *ptr = nullptr;
	...
	while (1) {
		...
		if (nodeA->isLeaf() && nodeB->isLeaf()) {
			//addPair(nodeA->triID(), nodeB->triID(), pairInter, pairInterIdx, rids, pairId);
			updateManifold(nodeA->triID(), nodeB->triID(), rids, manifoldPtr, manifoldIdx, ptr, tris, vtxs, trfA, trfB);
...
__global__ void
kernel_RigidRigidManifold(
	g_tri3f* tris, REAL3* vtxs, g_bvh_node* bvh_nodes, REAL margin,
	g_manifold *manifoldPtr, int* manifoldIdx,
	uint2* pairRig, uint* pairRigIdx,
	g_transf* rigs, int num)
{
	LEN_CHK(num);
	...
	trfBtoA.calc_from_homogenic(trfA, trfB);
	bvhQuery2(idx, trfBtoA, tris, vtxs, bvh_nodes, rp, manifoldPtr, manifoldIdx, trfA, trfB);
}
...
int checkRigidRigidManifoldGPU(REAL margin)
{
	int num = sRigPairs._length;
	int T = 1;
	int B = BPG(num, T);
	...
	g_mesh& km = sMeshes[0];
	kernel_RigidRigidManifold << <B, T >> > (
		km._tris, km._vtxs, km._bvh_nodes, margin,
		sManifoldPtr, sManifoldIdx,
		sRigPairs._dPairs, sRigPairs._dIdx,
		sRigids, num);
	getLastCudaError("kernel_RigidRigidManifold");


	int dummy = 0;
	cutilSafeCall(cudaMemcpy(&dummy, sManifoldIdx, sizeof(int), cudaMemcpyDeviceToHost));
	return dummy;
}
...
CU_FORCEINLINE __device__ void
triDistance(int idx,
	const g_box bx, const REAL3 &v0, const REAL3 &v1, const REAL3 &v2,
	g_tri3f* trisB, REAL3* vtxsB, g_bvh_node* rootB,
	REAL* minDist, REAL3* minPtPtr, int* minPtIdx)
{
	const g_bvh_node* stack[STACK_SIZE * 2];
	const g_bvh_node* nodeB = rootB;
	unsigned int stackPtr = 0;

	while (1) {
		if (boxDist(bx, nodeB->_box) > *minDist) {
			if (stackPtr == 0)
				break;
			else {
				nodeB = stack[--stackPtr];
		...
		if (nodeB->isLeaf()) {
			...
			REAL3 w0, w1, w2;
			getTriangleVtxs(trisB, vtxsB, Tb, w0, w1, w2);

			REAL3 p, q;

			double dist = triDist(v0, v1, v2, w0, w1, w2, p, q);
#if 0
			if (dist < minDist) {
				minDist = dist;
				ptA = p;
...
#if 0
			if (old > dist) {
				int pos = atomicAdd(minPtIdx, 1);
				minPtPtr[pos*2] = p;
				minPtPtr[pos*2+1] = q;
				if (pos > MAX_MINPT_NUM)
					printf("overflow!!!\n");
			}
#endif
...
CU_FORCEINLINE __device__ void
bvhDistance(int idx, const g_transf & trfA2B,
	g_tri3f* trisA, REAL3* vtxsA, g_bvh_node* rootA,
	g_tri3f* trisB, REAL3* vtxsB, g_bvh_node* rootB,
	REAL *minDist, REAL3 &ptA, REAL3 &ptB)
{
	const g_bvh_node* stack[STACK_SIZE * 2];
	const g_bvh_node* nodeA = rootA;
	const g_bvh_node* nodeB = rootB;
	unsigned int stackPtr = 0;
	...
	while (1) {
		...
		if (nodeA->isLeaf() && nodeB->isLeaf()) {
			...
			REAL3 v0, v1, v2;
			REAL3 w0, w1, w2;
			getTriangleVtxs(trisA, vtxsA, Ta, v0, v1, v2);
			w0 = trfA2B.getVertex(v0);
			w1 = trfA2B.getVertex(v1);
			w2 = trfA2B.getVertex(v2);
			getTriangleVtxs(trisB, vtxsB, Tb, v0, v1, v2);
...
__global__ void
kernel_RigidRigidDistance(
	g_tri3f* trisA, REAL3* vtxsA, g_bvh_node* rootA,
	g_tri3f* trisB, REAL3* vtxsB, g_bvh_node* rootB,
	REAL* rDist, g_transf* rigs, int num,
	REAL3* minPtPtr, int* minPtIdx)
{
	LEN_CHK(1);

	g_transf& trfA = rigs[0];
	g_transf& trfB = rigs[1];
	g_transf trfA2B = trfB.inverse() * trfA;

	REAL3 q, p;
	bvhDistance(0, trfA2B,
		trisA, vtxsA, rootA,
		trisB, vtxsB, rootB,
		rDist, q, p);
}
...
__device__ void
doTriRigidDistance(
	g_tri3f* trisA, REAL3* vtxsA,
	g_tri3f* trisB, REAL3* vtxsB, g_bvh_node* rootB,
	REAL* rDist,	g_transf &trfA2B, int idx,
	REAL3* minPtPtr, int* minPtIdx)
{

	REAL3 v0, v1, v2;
	getTriangleVtxs(trisA, vtxsA, idx, v0, v1, v2);
	...
	triDistance(idx,
		bx, w0, w1, w2,
		trisB, vtxsB, rootB,
		rDist,minPtPtr, minPtIdx);
}

__global__ void
kernel_TriRigidDistance2(
	g_tri3f* trisA, REAL3* vtxsA,
	g_tri3f* trisB, REAL3* vtxsB, g_bvh_node* rootB,
	REAL* rDist, g_transf* rigs, int num, int stride,
	REAL3* minPtPtr, int* minPtIdx)
{
	...
	for (int i = 0; i < stride; i++) {
		...
		doTriRigidDistance(trisA, vtxsA, trisB, vtxsB, rootB, rDist, trfA2B, j, minPtPtr, minPtIdx);
	}
}

__global__ void
kernel_TriRigidDistance(
	g_tri3f* trisA, REAL3* vtxsA,
	g_tri3f* trisB, REAL3* vtxsB, g_bvh_node* rootB,
	REAL* rDist, REAL3* rPtA, REAL3* rPtB,
	g_transf* rigs, int num,
	REAL3 *minPtPtr, int *minPtIdx)
{
	LEN_CHK(num);
...
__global__ void
kernel_BvhRigidDistance(
	g_tri3f* trisA, REAL3* vtxsA, g_bvh_node *rootA, int st,
	g_tri3f* trisB, REAL3* vtxsB, g_bvh_node* rootB,
	REAL* rDist, REAL3* rPtA, REAL3* rPtB,
	g_transf* rigs, int num)
{
	...
	bvhDistance(idx, trfA2B,
		trisA, vtxsA, rootA+st+idx,
		trisB, vtxsB, rootB,
		rDist, ptA, ptB);
...
//1 thread for all
void
checkRigidRigidDistanceGPU0(REAL &iDist, void *iPtA, void *iPtB)
{
	//input
	cudaMemcpy(gminDist, &iDist, sizeof(REAL), cudaMemcpyHostToDevice);
	cudaMemcpy(gptA, iPtA, sizeof(REAL)*3, cudaMemcpyHostToDevice);
	cudaMemcpy(gptB, iPtB, sizeof(REAL) * 3, cudaMemcpyHostToDevice);
	int dummyNum = 0;
	cudaMemcpy(sMinPtIdx, &dummyNum, sizeof(int), cudaMemcpyHostToDevice);

	g_mesh& kmA = sMeshes[0];
	g_mesh& kmB = sMeshes[1];

	kernel_RigidRigidDistance << <1, 1 >> > (
		kmA._tris, kmA._vtxs, kmA._bvh_nodes,
		kmB._tris, kmB._vtxs, kmB._bvh_nodes,
		gminDist, sRigids, 1, sMinPtPtr, sMinPtIdx);
	
	//output
	cudaMemcpy(&iDist, gminDist, sizeof(REAL), cudaMemcpyDeviceToHost);
	cudaMemcpy(iPtA, gptA, sizeof(REAL)*3, cudaMemcpyDeviceToHost);
	cudaMemcpy(iPtB, gptB, sizeof(REAL)*3, cudaMemcpyDeviceToHost);

	getLastCudaError("kernel_RigidRigidDistance");
}

// 1 thread for a triangle-bvh pair
void
checkRigidRigidDistanceGPU1(REAL& iDist, void* iPtA, void* iPtB)
{
	//input
	cudaMemcpy(gminDist, &iDist, sizeof(REAL), cudaMemcpyHostToDevice);
#if 0
	...
	int num = kmA._num_tri;
#if 0
	int T = 32;
	int stride = 3;
	int B = BPG(num, T, stride);

	kernel_TriRigidDistance2 <<<B, T >> > (
		kmA._tris, kmA._vtxs,
		kmB._tris, kmB._vtxs, kmB._bvh_nodes,
		gminDist, sRigids, num, stride,
		sMinPtPtr, sMinPtIdx);
#else
	int T = 96;
	int B = BPG(num, T);

	kernel_TriRigidDistance << <B, T >> > (
		kmA._tris, kmA._vtxs,
		kmB._tris, kmB._vtxs, kmB._bvh_nodes,
		gminDist, gptA, gptB,
		sRigids, num,
...
#if 0
	//get pts
	cudaMemcpy(&dummyNum, sMinPtIdx, sizeof(int), cudaMemcpyDeviceToHost);
	printf("dummyNum = %d\n", dummyNum);
	cudaMemcpy(iPtA, sMinPtPtr+dummyNum*2, sizeof(REAL) * 3, cudaMemcpyDeviceToHost);
	cudaMemcpy(iPtB, sMinPtPtr+dummyNum*2+1, sizeof(REAL) * 3, cudaMemcpyDeviceToHost);
#endif

	getLastCudaError("kernel_RigidRigidDistance");
}

// 1 thread for a sub-bvh / bvh pair
void
checkRigidRigidDistanceGPU2(REAL& iDist, void* iPtA, void* iPtB, int st, int num)
{
	//input
	cudaMemcpy(gminDist, &iDist, sizeof(REAL), cudaMemcpyHostToDevice);
	cudaMemcpy(gptA, iPtA, sizeof(REAL) * 3, cudaMemcpyHostToDevice);
	...
	kernel_BvhRigidDistance << <B, T >> > (
		kmA._tris, kmA._vtxs, kmA._bvh_nodes, st,
		kmB._tris, kmB._vtxs, kmB._bvh_nodes,
		gminDist, gptA, gptB,
		sRigids, num);
...
void pushMesh1(int meshNum)
{
	...
	cudaFuncSetCacheConfig(kernel_RigidRigidManifold, cudaFuncCachePreferL1);
	cudaFuncSetCacheConfig(kernel_TriRigidDistance, cudaFuncCachePreferL1);
	cudaFuncSetCacheConfig(kernel_BvhRigidDistance, cudaFuncCachePreferL1);
	...
	cudaMalloc(&sManifoldPtr, sizeof(g_manifold) * MAX_MANIFOLD_NUM);
	cudaMalloc(&sManifoldIdx, sizeof(int));
	int dummyNum = 0;
	cudaMemcpy(sManifoldIdx, &dummyNum, sizeof(int), cudaMemcpyHostToDevice);

	//for distance computing
	cudaMalloc(&gminDist, sizeof(REAL));
	cudaMalloc(&gptA, sizeof(REAL3));
	cudaMalloc(&gptB, sizeof(REAL3));

	cudaMalloc(&sMinPtPtr, sizeof(REAL3) * MAX_MINPT_NUM);
	cudaMalloc(&sMinPtIdx, sizeof(int));
	cudaMemcpy(sMinPtIdx, &dummyNum, sizeof(int), cudaMemcpyHostToDevice);
}
