...
void entity_module_init() {
	...
	hash_map_create(&entities_map, 1024, sizeof(eid), sizeof(Entity*), util_eid_compare, util_eid_hash);
}
...
static eid entity_create_ex(Mesh mesh, vec3 world_position, Quaternion world_rotation, vec3 world_scale, vec4 color, r64 mass, Collider* colliders,
	...
	if (is_fixed) {
		...
		(gm_mat3_inverse(&entity->inertia_tensor, &entity->inverse_inertia_tensor));
	}
	...
	(!hash_map_put(&entities_map, &entity->id, &entity));
	return entity->id;
}
...
void entity_destroy(Entity* entity) {
	...
	(hash_map_delete(&entities_map, &entity->id) == 0);
	free(entity);
}
