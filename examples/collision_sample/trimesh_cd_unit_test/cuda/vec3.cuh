...
CU_FORCEINLINE __host__ __device__ REAL setI(REAL3& a, int i, REAL x)
{
	if (i == 0)
		a.x = x;
	else if (i == 1)
...
CU_FORCEINLINE __host__ __device__ REAL3 absolute(const REAL3 &v)
{
	return make_REAL3(fabs(v.x), fabs(v.y), fabs(v.z));
}

CU_FORCEINLINE __host__ __device__ REAL3 dot3(const REAL3 &v, const REAL3& v0, const REAL3& v1, const REAL3& v2)
{
	return make_REAL3(dot(v, v0), dot(v, v1), dot(v, v2));
}
...
CU_FORCEINLINE __host__ __device__ void operator*=(REAL4& a, REAL b)
{
...
CU_FORCEINLINE __host__ __device__ int maxAxis4(const REAL4& v)
{
	int maxIndex = -1;
...
CU_FORCEINLINE __host__ __device__ int closestAxis4(const REAL4& v)
{
	REAL4 t = make_REAL4(fabsf(v.x), fabsf(v.y), fabsf(v.z), fabsf(v.w));
	return maxAxis4(t);
}
