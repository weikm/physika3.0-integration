...
//#pragma omp parallel for schedule(static)
	...
	void RigidRigidCollision(crigid* ra, crigid* rb, myTimer2& tr1, myTimer2& tr2, int& maxL, int& maxP)
	{
		std::vector<id_pair> pairset;
		{
			...
			const transf& trfA = ra->getTrf();
			const transf& trfB = rb->getTrf();
			ra->getMesh()->collide(rb->getMesh(), trfA, trfB, pairset);
			tr1.inc(omp_get_wtime() - t0);
		}
...
kmesh* initVolBunny(REAL scale = 1.0, vec3f &shift = vec3f())
{
	...
	kmesh* vbunny = new kmesh(numVtx, numTri, tris, vtxs, false);
...
#if defined(FOR_SDF) || defined(FOR_MIXED)
...
#ifdef FOR_PARTICLES
	{
		g_box += g_scene._particles.bound();
...
#ifdef FOR_VOLMESH
	{
		kmesh* kmA = initVolBunny();
		kmesh* kmB = initVolBunny(); // initBunny(c2file);
...
#if defined(FOR_SDF) || defined(FOR_TRIMESH)
	{
...
void getMinDistance()
{
	...
	minPairs.clear();
	g_scene.minPairs.clear();
	REAL dist = bodyA->getMinDistance(bodyB, g_scene.minPairs);
	
	double tdelta = omp_get_wtime() - tstart;
	printf("MinDistQuery = %lf at %2.5f s\n", dist, tdelta);
	totalQuery += tdelta;
}
...
void checkCollision()
{
#ifdef FOR_PARTICLES
	{
		...
		g_scene._particles.checkCollision(g_scene.cdPairs);
		double tdelta = omp_get_wtime() - tstart;
...
#ifdef FOR_SDF
	{
		...
		double tdelta = omp_get_wtime() - tstart;
		printf("checkCollison (%zd pairs) at %2.5f s\n", g_scene.sdfPairs.size() / 2, tdelta);
		totalQuery += tdelta;
		return;
	}
#endif

#if defined(FOR_VOLMESH) || defined(FOR_TRIMESH)
	{
		double tstart = omp_get_wtime();
		crigid* bodyA = g_scene.getRigid(0);
		crigid* bodyB = g_scene.getRigid(1);
...
void getMinDistanceGPU()
{
	...
	//gpuTimerBegin();
	REAL dist = bodyA->getMinDistanceGPU(bodyB, g_scene.minPairs);
	//gpuTimerEnd("MinDistQueryGPU", true);

	double tdelta = omp_get_wtime() - tstart;
	printf("MinDistQueryGPU = %lf at %2.5f s\n", dist, tdelta);
	totalQuery += tdelta;
