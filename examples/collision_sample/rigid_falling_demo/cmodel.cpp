...
auto             scene  = Physika::World::instance().createScene();
auto             solver = Physika::World::instance().createSolver<Physika::MeshMeshCollisionSolver>();
Physika::Object* bunnies[150];
...
public:
    ...
    void push2GPU()
    {
        printf("cpu size = %zd, %zd, %zd, %zd, %zd, %zd\n",
               ...
               sizeof(vec3f),
               sizeof(matrix3f),
               sizeof(Physika::bvh_node),
               sizeof(transf),
               sizeof(Physika::qbvh_node));
    ...
    // TODO(use collsion solver to check collision)
...
triMesh* initCyl()
{
    ...
    triMesh* cyl = new triMesh(splits * 2, numTri, tris, points, true);
    g_scene.addMesh(cyl);
    return cyl;
}
...
crigid* shootMesh(triMesh* km, const vec3f& startPos, const vec3f& dstPos)
{
    float      mass                   = 4.f;
...
void initModel(const char* cfile)
{
    ...
    for (int i = 0; i < 150; i++)
    {
        bunnies[i] = Physika::World::instance().createObject();
        bunnies[i]->addComponent<Physika::CollidableTriangleMeshComponent>();
        scene->addObject(bunnies[i]);
        solver->attachObject(bunnies[i]);
    }
}
...
extern void beginDraw(BOX<REAL>&);
extern void endDraw();
