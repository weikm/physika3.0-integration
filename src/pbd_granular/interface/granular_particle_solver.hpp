 ...
 *              The file defines the GranularPhase enum, GranularComponent struct, and the GranularParticleSolver class which inherits from the base Solver class.
...
#ifndef GRANULAR_PARTICLE_SOLVER_HPP
#define GRANULAR_PARTICLE_SOLVER_HPP
 ...
 */
struct GranularParticleComponent

{
    ...
    unsigned int  m_num_collisions;         // how many particles are in collision
    unsigned int* m_collision_particle_id;  // The particle id of collision particles (according to m_device_pos).
    float*        m_move_direction;         // data layout: x, y, z. The move direction for partiles to avoid collision.
    float*        m_move_distance;          // data layout: single float. The move distance for particles to avoid collision
    float*        m_collision_force;        // The force for partiles from collision.
     ...
     *
     * Using GranularParticleSolver::set...() methods instead of this function is recommended
     */
    // ~GranularParticleComponent();
};
 ...
 */
class GranularParticleSolver : public Solver
{
public:
    struct SolverConfig
    {
...
private:
    ...
    //GranularParticleComponent* m_granular_particle_component;
