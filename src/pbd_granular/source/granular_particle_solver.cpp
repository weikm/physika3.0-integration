...
#include "granular_particle_solver.hpp"
...
void GranularParticleComponent::reset()
{
    freeMemory();
    m_host_pos.resize(0);
    m_host_vel.resize(0);
...
void GranularParticleComponent::addInstance(float particle_radius, float3 lb, float3 size, float3 vel_start)
{
...
void GranularParticleComponent::addInstance(float particle_radius, std::vector<float3>& init_pos)
{
    float invmass   = 1.0f / pow(particle_radius / 0.3, 3);
...
void GranularParticleComponent::initialize()
{
    freeMemory();
    if (m_bInitialized)
    {
...
void GranularParticleComponent::freeMemory()
{
...
GranularParticleSolver::GranularParticleSolver()
    : m_is_init(false), m_granular_particle(nullptr), m_cur_time(0), m_host_pos(nullptr), m_collision_particle_id(nullptr), m_device_cell_end(nullptr), m_device_cell_start(nullptr), m_device_delta_pos(nullptr), m_device_grid_particle_hash(nullptr), m_device_predicted_pos(nullptr)
{
...
bool GranularParticleSolver::initialize()
{
    if (m_is_init)
        return true;
    if (m_granular_particle == nullptr)
    {
        std::cout << "ERROR: Must set granular particle object first.\n";
        return false;
    }
    if (m_granular_particle->hasComponent<GranularParticleComponent>() == false)
    {
        std::cout << "ERROR: granular particle object has no granular component.\n";
        return false;
    }
    GranularParticleComponent* granular_component = m_granular_particle->getComponent<GranularParticleComponent>();
...
bool GranularParticleSolver::isInitialized() const
{
    return m_is_init;
}

bool GranularParticleSolver::reset()
{
    m_is_init             = false;
    m_config.m_dt         = 0.0;
    m_config.m_total_time = 0.0;
    m_granular_particle   = nullptr;
    m_cur_time            = 0.0;
    if (m_granular_particle != nullptr)
    {
        m_granular_particle->getComponent<GranularParticleComponent>()->reset();
    }
    return true;
}

bool GranularParticleSolver::step()
{
    if (!m_is_init)
    {
    ...
    if (!m_granular_particle->hasComponent<GranularParticleComponent>())
        return false;
    GranularParticleComponent* granular_component = m_granular_particle->getComponent<GranularParticleComponent>();
...
bool GranularParticleSolver::run()
{
    if (!m_is_init)
    {
        return false;
    }
    if (!m_granular_particle)
    {
        return false;
    }
...
bool GranularParticleSolver::isApplicable(const Object* object) const
{
    if (!object)
        return false;

    return object->hasComponent<GranularParticleComponent>();
}

bool GranularParticleSolver::attachObject(Object* object)
{
    if (!object)
        return false;
    ...
    if (object->hasComponent<GranularParticleComponent>())
    {
...
bool GranularParticleSolver::detachObject(Object* object)
{
    if (!object)
        return false;

    if (m_granular_particle == object)
    {
        m_granular_particle = nullptr;
        return true;
    }
...
void GranularParticleSolver::clearAttachment()
{
    m_granular_particle = nullptr;
}

void GranularParticleSolver::config(SolverConfig& config)
{
...
bool GranularParticleSolver::setWorldBoundary(float lx, float ly, float lz, float ux, float uy, float uz)
{
    if (!m_is_init)
        return false;
    if (!m_granular_particle->hasComponent<GranularParticleComponent>())
...
void GranularParticleSolver::getWorldBoundary(float& lx, float& ly, float& lz, float& ux, float& uy, float& uz)
{   
...
void GranularParticleSolver::setStiffness(float& stiffness)
{
    if (stiffness > 1.f)
        stiffness = 1.f;
    else if (stiffness <= 0)
...
void GranularParticleSolver::getStiffness(float& stiffness)
{
    stiffness = m_params->m_stiffness;
}

void GranularParticleSolver::setSolverIteration(unsigned int& iteration)
{
    if (iteration <= 0)
        iteration = 1;
...
void GranularParticleSolver::getSolverIteration(unsigned int& iteration)
{
    iteration = m_params->m_max_iter_nums;
}

void GranularParticleSolver::setGravity(const float& gravity)
{
    m_params->m_gravity = make_float3(0, gravity, 0);
}

void GranularParticleSolver::setGravity(const float& x, const float& y, const float& z)
{
    m_params->m_gravity = make_float3(x, y, z);
}

void GranularParticleSolver::getGravity(float& x, float& y, float& z)
{
...
void GranularParticleSolver::setSleepThreshold(float threshold)
{
    if (threshold < 0.f)
        threshold = 0.f;
...
void GranularParticleSolver::getSleepThreshold(float& threshold)
{
    threshold = m_params->m_sleep_threshold;
}

void GranularParticleSolver::setWorldOrigin(const float& x, const float& y, const float& z)
{
    m_params->m_world_origin = make_float3(x, y, z);
}

void GranularParticleSolver::getWorldOrigin(float& x, float& y, float& z)
{
...
void GranularParticleSolver::setHeightField(const std::vector<float>& height, const float unit_height, const int height_x_num, const int height_z_num)
{
...
void GranularParticleSolver::setStaticFrictionCoeff(float& staticFriction)
{
    m_params->m_static_fricton_coeff = std::clamp(staticFriction, 0.f, 1.f);
}

void GranularParticleSolver::getStaticFrictionCoeff(float& staticFriction)
{
    staticFriction = m_params->m_static_fricton_coeff;
}

void GranularParticleSolver::setDynamicFrictionCoeff(float& dynamicFriction)
{
    m_params->m_dynamic_fricton_coeff = std::clamp(dynamicFriction, 0.f, 1.f);
}

void GranularParticleSolver::getDynamicFrictionCoeff(float& dynamicFriction)
{
    dynamicFriction = m_params->m_dynamic_fricton_coeff;
}

void GranularParticleSolver::_finalize()
{
    ...
    {
        if (m_granular_particle->hasComponent<GranularParticleComponent>())
            m_granular_particle->getComponent<GranularParticleComponent>()->freeMemory();
    }
...
bool GranularParticleSolver::setParticlePosition(const std::vector<float>& position)
{
    if (!m_granular_particle->hasComponent<GranularParticleComponent>())
        return false;
    GranularParticleComponent* granular_component = m_granular_particle->getComponent<GranularParticleComponent>();
...
bool GranularParticleSolver::setParticleVelocity(const std::vector<float>& velocity)
{
    if (!m_granular_particle->hasComponent<GranularParticleComponent>())
        return false;
    GranularParticleComponent* granular_component = m_granular_particle->getComponent<GranularParticleComponent>();
...
bool GranularParticleSolver::setParticlePhase(const std::vector<float>& particlePhase)
{
    if (!m_granular_particle->hasComponent<GranularParticleComponent>())
    {
        GranularParticleComponent* granular_component = m_granular_particle->getComponent<GranularParticleComponent>();
...
bool GranularParticleSolver::setParticleCollisionForcePtr(float* collisionForce)
{
    if (m_granular_particle != nullptr)
    {
        if (!m_granular_particle->hasComponent<GranularParticleComponent>())
...
float* GranularParticleSolver::getParticlePositionPtr()
{
    if (m_granular_particle != nullptr)
    {
        if (!m_granular_particle->hasComponent<GranularParticleComponent>())
...
float* GranularParticleSolver::getParticleVelocityPtr()
{
    if (m_granular_particle != nullptr)
    {
        if (!m_granular_particle->hasComponent<GranularParticleComponent>())
...
float* GranularParticleSolver::getParticlePhasePtr()
{
    if (m_granular_particle != nullptr)
    {
        if (!m_granular_particle->hasComponent<GranularParticleComponent>())
...
float* GranularParticleSolver::getParticleCollisionForcePtr()
{
    if (m_granular_particle != nullptr)
    {
        if (!m_granular_particle->hasComponent<GranularParticleComponent>())
...
void GranularParticleSolver::setParticleRadius(const float& particleRadius)
{
    if (!m_is_init)
        return;
    if (particleRadius <= 0)
...
void GranularParticleSolver::getParticleRadius(float& particleRadius)
{
    if (!m_is_init)
        particleRadius = 0;
...
void GranularParticleSolver::handleCollision(unsigned int* collision_particle_id, float* moveDirection, float* moveDistance, unsigned int collision_num)
{
    if (collision_num == 0)
        return;
...
void GranularParticleSolver::writeToPly(const int& step_id)
{
    if (!m_granular_particle->hasComponent<GranularParticleComponent>())
        return;
    GranularParticleComponent* granular_component = m_granular_particle->getComponent<GranularParticleComponent>();
...
void GranularParticleSolver::freeParticleMemory(const int& numParticles)
{
    if (!m_granular_particle->hasComponent<GranularParticleComponent>())
        return;
    GranularParticleComponent* granular_component = m_granular_particle->getComponent<GranularParticleComponent>();
    if (granular_component == nullptr)
        return;
    granular_component->~GranularParticleComponent();
...
void GranularParticleSolver::mallocParticleMemory(const int& numParticles)
{
    if (!m_granular_particle->hasComponent<GranularParticleComponent>())
        return;
    GranularParticleComponent* granular_component = m_granular_particle->getComponent<GranularParticleComponent>();
