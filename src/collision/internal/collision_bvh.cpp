...
static std::vector<Physika::triMesh*>* ptCloth;

static void mesh_id(int id, std::vector<Physika::triMesh*>& m, int& mid, int& fid)
{
    fid = id;
...
static bool covertex(int id1, int id2)
{
    ...
    Physika::tri3f& f1 = (*ptCloth)[mid1]->_tris[fid1];
    Physika::tri3f& f2 = (*ptCloth)[mid2]->_tris[fid2];
...
void bvh_node::construct(unsigned int id, BOX<REAL>* s_fboxes)
{
   _child = id;
   _box   = s_fboxes[id];
}

void bvh_node::construct(unsigned int* lst, unsigned int num, vec3f* s_fcenters, BOX<REAL>* s_fboxes, bvh_node*& s_current)
{
...
void bvh_node::refit(BOX<REAL>* s_fboxes)
{
   if (isLeaf())
   {
...
void bvh_node::sprouting2(bvh_node* other, front_list& append, std::vector<TrianglePair>& ret)
{
   if (isLeaf() && other->isLeaf())
   {
...
void bvh::refit(BOX<REAL>* s_fboxes)
{
   root()->refit(s_fboxes);
}

void bvh::construct(std::vector<triMesh*>& ms) 
{
   BOX<REAL> total;
...
void bvh::construct(triMesh* mesh)
{
   BOX<REAL> total;

   for (int i = 0; i < mesh->_num_vtx; i++)
   {
       total += mesh->_vtxs[i];
   }
   ...
   for (int j = 0; j < mesh->_num_tri; j++)
   {
       tri3f& f  = mesh->_tris[j];
       vec3f& p1 = mesh->_vtxs[f.id0()];
       vec3f& p2 = mesh->_vtxs[f.id1()];
       vec3f& p3 = mesh->_vtxs[f.id2()];
...
void bvh::refit(std::vector<triMesh*>& ms) 
{
   assert(s_fboxes);
...
void bvh::self_collide(front_list& f, std::vector<triMesh*>& c)
{
   f.clear();
...
void bvh::rayCasting(const vec3f& pt, const vec3f& dir, triMesh* km, REAL& ret) {}
// faster with fewer division
void bvh::rayCasting2(const vec3f& pt, const vec3f& dir, const vec3f& dir2, triMesh* km, REAL& ret) {}
// faster with no recursion
void bvh::rayCasting3(const vec3f& pt, const vec3f& dir, const vec3f& dir2, triMesh* km, REAL& ret) {}
