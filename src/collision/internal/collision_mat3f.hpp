...
#include "collision/internal/collision_quaternion.hpp"
...
public:
    ...
    matrix3f(REAL entry00, REAL entry01, REAL entry02, REAL entry10, REAL entry11, REAL entry12, REAL entry20, REAL entry21, REAL entry22)
    {
    ...
    {
        return matrix3f(
            _data[0] * s.x, _data[3] * s.y, _data[6] * s.z, _data[1] * s.x, _data[4] * s.y, _data[7] * s.z, _data[2] * s.x, _data[5] * s.y, _data[8] * s.z);
    }
    ...
    static matrix3f rotation(const quaternion& q)
    {
        ...
        return matrix3f(
            float(1.0) - (yy + zz), xy - wz, xz + wy, xy + wz, float(1.0) - (xx + zz), yz - wx, xz - wy, yz + wx, float(1.0) - (xx + yy));
    }
    ...
    void getRotation(quaternion& q) const
    {
        ...
        {
            int i = operator()(0, 0) < operator()(1, 1) ? (operator()(1, 1) < operator()(2, 2) ? 2 : 1) : (operator()(0, 0) < operator()(2, 2) ? 2 : 0);
            int j = (i + 1) % 3;
            int k = (i + 2) % 3;
