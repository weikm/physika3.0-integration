...
#include "collision/internal/collidable_mesh.cuh"
#include "collision/internal/collision_pair.cuh"
#include "collision/internal/collision_tri3.cuh"
#include "collision/internal/collision_bvh.cuh"
#include "collision/internal/collision_tricontact.cuh"
...
inline __device__ int addPair(uint a, uint b, uint2* pairs, uint* idx)
{
    if (*idx < MAX_PAIR_NUM)
    {
...
// isVF  0:VF  1:EE
inline __device__ int addPairCCD(uint a, uint b, uint isVF, uint id1, uint id2, uint id3, uint id4, float d, int* t, uint2* pairs, int4* dv, int* VF_EE, float* dt, int* CCDres, uint* idx)
{
    if (*idx < MAX_PAIR_NUM)
    {
        uint offset     = atomicAdd(idx, 1);
...
// isVF  0:VF  1:EE
inline __device__ int addPairDCD(uint a, uint b, uint isVF, uint id1, uint id2, uint id3, uint id4, float d, int* t, uint2* pairs, int4* dv, int* VF_EE, float* dt, int* CCDres, uint* idx)
{
    if (*idx < MAX_PAIR_NUM)
    {
        uint offset     = atomicAdd(idx, 1);
...
inline __device__ void sprouting(int left, int right, int* bvhA, g_box* bxsA, int* bvhB, g_box* bxsB, uint4* front, uint* frontIdx, uint2* pairs, uint* pairIdx, bool update, uint ptr, tri3f* Atris)
{
        ...
        if (isLeaf(left, bvhA) && isLeaf(right, bvhB))
        {
            ...
            if (overlaps(left, right, bxsA, bxsB))
            {
                if (!covertex(getTriID(left, bvhA), getTriID(right, bvhB), Atris))
                {
                    ...
                    printf("%d   %d\n", getTriID(left, bvhA), getTriID(right, bvhB));
                }
            }
        }
        else
...
// TODO(wangwei)
inline __device__ void sproutingAdaptive(int left, int right, int* bvhA, g_box* bxsA, int* bvhB, g_box* bxsB, uint4* front, uint* frontIdx, uint2* pairs, uint* pairIdx, bool update, uint ptr)
{
}
...
__global__ void kernelPropogate(
    uint4* front,
    uint*  frontIdx,
    int    num,
    int*   bvhA,
    ...
    bool*  flags)
{
    ...
    for (int i = 0; i < stride; i++)
    {
        ...
        //printf("num: %d  j:%d   l :%d   r:  %d\n", num,j, front[j].x, front[j].y);
...
__device__ REAL newtons_method(REAL a, REAL b, REAL c, REAL d, REAL x0, int init_dir)
{
    if (init_dir != 0)
    {
...
__device__ REAL signed_ee_distance(const REAL3& x0, const REAL3& x1, const REAL3& y0, const REAL3& y1, REAL3* n, REAL* w);

__device__ REAL signed_vf_distance(const REAL3& x,
                                   const REAL3& y0,
                                   const REAL3& y1,
                                   const REAL3& y2,
                                   REAL3*       n,
...
dnf_filter(REAL3& a0, REAL3& b0, REAL3& c0, REAL3& d0, REAL3& a1, REAL3& b1, REAL3& c1, REAL3& d1)
{
    ...
    int*   t)
{
    ...
    {
        doImpactVF(
            p[st], q[0], q[1], q[2], pp[st], qq[0], qq[1], qq[2], time);
    }

    // VF
    for (int st = 0; st < 3; st++)
    {
        doImpactVF(q[st], p[0], p[1], p[2], qq[st], pp[0], pp[1], pp[2], time);
    }
    ...
    for (int st = 0; st < 3; st++)
    {
        ...
        for (int ss = 0; ss < 3; ss++)
        {
            ...
            int ind3 = (st + 1) % 3;
            doImpactEE(
                cx[idx0[ind0]], cx[idx0[ind1]], cx[idx1[ind2]], cx[idx1[ind3]], cx0[idx0[ind0]], cx0[idx0[ind1]], cx0[idx1[ind2]], cx0[idx1[ind3]], time);
        }
    }
    ...
    REAL*  thickness)
{
    ...
    for (int st = 0; st < 3; st++)
    {
        ...
        if ((time >= 0 && time <= 1) || (d < thickness[0] && d >= 0))
        {
            ...
            addPairCCD(fid1, fid2, 0, idx0[st], idx1[0], idx1[1], idx1[2], d, t, pairRets, dv, VF_EE, dist, CCDres, pairIdx);
        }
    ...
    for (int st = 0; st < 3; st++)
    {
        ...
        if ((time >= 0 && time <= 1) || (d < thickness[0] && d >= 0))
        {
            addPairCCD(fid1, fid2, 0, idx1[st], idx0[0], idx0[1], idx0[2], d, t, pairRets, dv, VF_EE, dist, CCDres, pairIdx);
        }
    ...
    for (int st = 0; st < 3; st++)
    {
        ...
        for (int ss = 0; ss < 3; ss++)
        {
            ...
            REAL d = doProximityEE(
                cx[idx0[ind0]], cx[idx0[ind1]], cx[idx1[ind2]], cx[idx1[ind3]], thickness);
            ...
            if ((time >= 0 && time <= 1) || (d < thickness[0] && d >= 0))
            {
                ...
                addPairCCD(fid1, fid2, 1, idx0[ind0], idx0[ind1], idx1[ind2], idx1[ind3], d, t, pairRets, dv, VF_EE, dist, CCDres, pairIdx);
            }
...
__global__ void kernelGetCollisionsCCD(
    uint2*  pairs,
    int    num,
    REAL3* cx,
    REAL3* cx0,
    ...
    int    stride)
{
    ...
    for (int i = 0; i < stride; i++)
    {
        ...
        tri_CCD2(t1.id0(), t1.id1(), t1.id2(), t2.id0(), t2.id1(), t2.id2(), cx, cx0, pairRets, dv, VF_EE, dist, CCDres, t, pairIdx, fid1, fid2, thickness);
    }
          ...
          uint*           pairInterIdx)
{
    ...
    while (1)
    {
        ...
        if (node->isLeaf())
        {
            addPair(idx, node->triID(), pairInter, pairInterIdx);
...
// TODO(wangwei)
int g_pair::getCollisions(bool self, struct _g_pairCCD& ret, int* time, float* thickness)
{
    int num = length();
    printf("pair = %d\n", num);
...
#else
    BLK_PAR3(num, stride, getBlkSize(( void* )kernelGetCollisionsCCD));
#endif
    ...
    kernelGetCollisionsCCD<<<B, T>>>(_dPairs, num, theCloth._dx, theCloth._dx0, theCloth._df, ret._dPairs, ret._dv, ret._dVF_EE, ret.dist, ret.CCD_res, ret._dIdx, tem_time, gthickness, stride);

    getLastCudaError("kernelGetCollisionsCCD");
...
void pushMesh2GPUDCD(int numFace, int numVert, void* faces, void* nodes)
{
    theCloth.init();
...
void updateMesh2GPUDCD(void* nodes)
{
...
void refitBVH_SerialDCD(bool isCloth, int length)
{
    refit_serial_kernel<<<1, 1, 0>>>(theBVH[isCloth]._bvh, theBVH[isCloth]._bxs, theBVH[isCloth]._triBxs, theBVH[isCloth]._cones, theBVH[isCloth]._triCones, length == 0 ? theBVH[isCloth]._num : length);
...
void refitBVH_ParallelDCD(bool isCloth, int st, int length)
{
    BLK_PAR(length);
...
void refitBVHDCD(bool isCloth)
{
    ...
    for (int i = theBVH[isCloth]._max_level - 1; i >= 0; i--)
    {
        ...
        if (i < 5)
        {
            refitBVH_SerialDCD(isCloth, length + st);
            break;
        }
        else
        {
            refitBVH_ParallelDCD(isCloth, st, length);
        }
    }
}
...
__global__ void kernelGetCollisionsDCD(
    uint2* pairs,
    int    num,
    REAL3* cx,
    tri3f* ctris,
...
int _g_pair::getCollisionsDCD(bool self, _g_pair& ret)
{
    int num = length();
    ...
    BLK_PAR3(num, stride, 32);
#else
    BLK_PAR3(num, stride, getBlkSize(( void* )kernelGetCollisionsDCD));
#endif

    kernelGetCollisionsDCD<<<B, T>>>(_dPairs, num, theCloth._dx, theCloth._df, ret._dPairs, ret._dIdx, stride);

    getLastCudaError("kernelGetCollisionsDCD");
...
int getCollisionsDCDGPU(int* rets)
{
    bool update = true;
    int  len    = 0;
    ...
    len = thePairs[1].getCollisionsDCD(true, retPairs);
    cudaThreadSynchronize();

    if (len > 0)
    {
        cudaMemcpy(rets, retPairs._dPairs, sizeof(int) * 2 * len, cudaMemcpyDeviceToHost);
    }
    return len;
}
