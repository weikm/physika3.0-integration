...
public:
    ...
    {
        if (p[0] < _min[0] || p[0] > _max[0])
            return false;
        if (p[1] < _min[1] || p[1] > _max[1])
            return false;
        if (p[2] < _min[2] || p[2] > _max[2])
            return false;
    ...
    FORCEINLINE T width() const
    {
        return _max[0] - _min[0];
    }
    FORCEINLINE T height() const
    {
        return _max[1] - _min[1];
    }
    FORCEINLINE T depth() const
    {
    ...
    FORCEINLINE void enlarge(T thickness)
    {
        _max += vec3f(thickness, thickness, thickness);
        _min -= vec3f(thickness, thickness, thickness);
    }

    FORCEINLINE const vec3f& getMax() const
    {
        return _max;
    }
    FORCEINLINE const vec3f& getMin() const
    {
        return _min;
    }
    FORCEINLINE void setMax(vec3f& v)
    {
        _max = v;
    }
    FORCEINLINE void setMin(vec3f& v)
    {
        _min = v;
    }
