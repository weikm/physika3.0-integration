#include "collision/internal/collision_util.hpp"
#include "collision/internal/cubic.hpp"
...
static inline bool _insideTriangle(const vec3f& a, const vec3f& b, const vec3f& c, const vec3f& p, vec3f& baryc)
{
    ...
    if ((wa = (db.cross(dc)).dot(n)) < 0.0f)
        return false;
    if ((wb = (dc.cross(da)).dot(n)) < 0.0f)
        return false;
    if ((wc = (da.cross(db)).dot(n)) < 0.0f)
        return false;
...
*/
static inline bool LineLineIntersect(
    const vec3f& p1,
    const vec3f& p2,
    const vec3f& p3,
    const vec3f& p4,
 ...
 */
static inline void _equateCubic_VF(const vec3f& a0, const vec3f& ad, const vec3f& b0, const vec3f& bd, const vec3f& c0, const vec3f& cd, const vec3f& p0, const vec3f& pd, double& a, double& b, double& c, double& d)
{
    /*
 ...
 * Computes the coefficients of the cubic equations from the geometry data.
 */
static inline void _equateCubic_VE(
    const vec3f& a0,
    const vec3f& ad,
    const vec3f& b0,
    const vec3f& bd,
 ...
 * Computes the coefficients of the cubic equations from the geometry data.
 */
static inline void _equateCubic_EE(const vec3f& a0, const vec3f& ad, const vec3f& b0, const vec3f& bd, const vec3f& c0, const vec3f& cd, const vec3f& d0, const vec3f& dd, double& a, double& b, double& c, double& d)
{
    /*
     * For definitions & notation refer to the semester thesis doc.
     */
...
///////////////////////////////////////////////////////////////////////////////////////////////
static const double flat_tol     = 1e-6;
static const double one_div_tree = 1. / 3.;
...
static bool findRoots(bezier_info crv, double* roots, int& num)
{
    ...
    while (!EMPTY(idx))
    {
        ...
        if ((crv.a > 0 && crv.d < 0) || (crv.a < 0 && crv.d > 0))
        {  // must have > 1 root
            if (crv.isFlat())
            {
        ...
        if ((crv.a > 0 && crv.d > 0 && crv.c > 0 && crv.b > 0) || (crv.a < 0 && crv.d < 0 && crv.c < 0 && crv.b < 0))
...
static inline vec3f norm(const vec3f& p1, const vec3f& p2, const vec3f& p3)
{
    return (p2 - p1).cross(p3 - p1);
}

static void find_coplanarity_times(
    const vec3f& ta0,
    const vec3f& tb0,
    const vec3f& tc0,
    const vec3f& td0,
...
static double
Intersect_VF2(const vec3f& ta0, const vec3f& tb0, const vec3f& tc0, const vec3f& ta1, const vec3f& tb1, const vec3f& tc1, const vec3f& q0, const vec3f& q1, vec3f& qi, vec3f& baryc)
{
    double roots[3];
    int    num = 0;
...
static double
Intersect_EE1(const vec3f& ta0, const vec3f& tb0, const vec3f& tc0, const vec3f& td0, const vec3f& ta1, const vec3f& tb1, const vec3f& tc1, const vec3f& td1, vec3f& qi)
{
...
double CollisionUtil::Intersect_EE(const vec3f& ta0, const vec3f& tb0, const vec3f& tc0, const vec3f& td0, const vec3f& ta1, const vec3f& tb1, const vec3f& tc1, const vec3f& td1, vec3f& qi)
{
...
double CollisionUtil::Intersect_VF(const vec3f& ta0, const vec3f& tb0, const vec3f& tc0, const vec3f& ta1, const vec3f& tb1, const vec3f& tc1, const vec3f& q0, const vec3f& q1, vec3f& qi, vec3f& baryc)
{
    /* Default value returned if no collision occurs */
    double collisionTime = -1.0f;
