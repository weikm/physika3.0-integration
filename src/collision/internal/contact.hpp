...
public:
    manifoldPoint()
        : m_userPersistentData(0), m_lateralFrictionInitialized(false), m_appliedImpulse(0.f), m_appliedImpulseLateral1(0.f), m_appliedImpulseLateral2(0.f), m_contactMotion1(0.f), m_contactMotion2(0.f), m_contactCFM1(0.f), m_contactCFM2(0.f), m_lifeTime(0)
    {
    }

    manifoldPoint(const vec3f& pointA, const vec3f& pointB, const vec3f& normal, float distance)
        : m_localPointA(pointA), m_localPointB(pointB), m_normalWorldOnB(normal), m_distance1(distance), m_combinedFriction(float(0.)), m_combinedRollingFriction(float(0.)), m_combinedRestitution(float(0.)), m_userPersistentData(0), m_lateralFrictionInitialized(false), m_appliedImpulse(0.f), m_appliedImpulseLateral1(0.f), m_appliedImpulseLateral2(0.f), m_contactMotion1(0.f), m_contactMotion2(0.f), m_contactCFM1(0.f), m_contactCFM2(0.f), m_lifeTime(0)
    {
    }
...
class manifold
{
    ...
    const crigid* m_body0;
    const crigid* m_body1;
...
public:
    ...
    manifold(const crigid* body0, const cplane* pln, const crigid* body1, int, float contactBreakingThreshold, float contactProcessingThreshold)
        : m_body0(body0), m_plane(pln), m_body1(body1), m_cachedPoints(0), m_contactBreakingThreshold(contactBreakingThreshold), m_contactProcessingThreshold(contactProcessingThreshold)
    {
    }

    __forceinline const crigid* getBody0() const
    {
        return m_body0;
    }
    __forceinline const cplane* getPlane() const
    {
        return m_plane;
    }
    __forceinline const crigid* getBody1() const
    {
        return m_body1;
    }
    ...
    {
        assert(pt.m_userPersistentData == nullptr);
    }
